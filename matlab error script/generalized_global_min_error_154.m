...
% original_output_results_directory = '/media/jpldev/E/pix2pix_aws_output/';
original_output_results_directory = 'C:/Users/Nick/Desktop/Alex/originals/';
% results_directory = '/media/jpldev/E/pix2pix_aws_output_newpostprocess/';
results_directory = 'C:/Users/Nick/Desktop/Alex/postprocessing/';
save_directory = 'C:/Users/Nick/Desktop/Alex/154_reports/';
% gt_folder='154_Helicopter_GT/';
gt_folder='C:/Users/Nick/Desktop/Alex/154_Helicopter_GT/';

overlay_save_dir = strcat(save_directory, 'temp/');
mkdir(overlay_save_dir);

result_folders = dir(results_directory);
total_results = size(result_folders, 1);
...
for result_folder_index = 3:total_results-1
    result_row = 1;
    
    % Current result folder for DNN output
    current_result_folder = result_folders(result_folder_index).name;
    result_videos = dir(strcat(results_directory, current_result_folder));
    
    global_min_epochs = dir(strcat(results_directory, current_result_folder, '/', result_videos(3).name, '/', current_result_folder,'/'));
    ...
    for epoch_index = 3:total_epochs
    ...
    for current_epoch_index = 1:size(sorted_epochs,1)
        ...
        for i = 1:size(gt_images_dir, 1)
            try
                index = num2str(i);
                dnn_folder_name = current_result_folder;
                ...
                confidence_map_sb =  strcat(original_output_results_directory, '/', current_result_folder, '/', output_case, '/', dnn_folder_name, '/', epoch, '_net_G_', output_case, '/images/original_res_output/', original_frame_name);
                dnn_output_sb = strcat(results_directory, '/', current_result_folder, '/', output_case, '/', dnn_folder_name, '/', epoch, '_net_G_', output_case, '/images/final_output/postprocess/', original_frame_name);
